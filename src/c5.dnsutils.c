...
const unsigned char *c5_report_name(const unsigned char *msg,
  ...
  if ((*p & 0xC0) == 0xC0) {
    ...
    if (*p) {
      printf(".");
      return c5_report_name(msg, p, end);
    } else {
      return p + 1;
    }
  }
...
int c5_dnsutils_init(int argc, char *argv[]) {
  ...
  while (*h) {
    ...
    *len = p - len - 1; // Set the length of the current label
    if (*h == '.')
      h++;
  }
  *p++ = 0;
  ...
  *p++ = 0x01; /* QCLASS */
  ...
  c5_print_dns_message(query, query_size);
  ...
  c5_print_dns_message(read, bytes_received);
  printf("\n");
...
void test_c5_dnsutils(int argc, char **argv) {
  assert(!c5_dnsutils_init(argc, argv));
}
